...
from model.yolo import ClassificationModel, Detect, DetectionModel, SegmentationModel
...
def add_tflite_metadata(file, metadata, num_outputs, tflite_support=None):
...
def run(
        data=ROOT / 'data/cbdata.yaml',  # 'dataset.yaml path'
        weights=ROOT / 'exp3.weights.best.pt',  # weights path
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/cbdata.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'exp3.weights.best.pt', help='model.pt path(s)')
